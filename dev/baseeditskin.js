...
function checkCache(img) {
  ...
  function load(setup) {
    ...
    loader.add('loadgame','/assets/char/loadgame.png');
    loader.add('dauthan_click','/assets/char/myTexture2dPea1-resources.assets-859.png');
    loader.add('dauthan','/assets/char/myTexture2dPea0-resources.assets-813.png');
  
    loader.add('change','/assets/char/change.png');
    loader.add('change_click','/assets/char/change_click.png');
  
    loader.add('skill','/assets/char/skill.png');
    loader.add('skill_click','/assets/char/skill_click.png');
  
    loader.add('bg_attack','/assets/char/bg_attack.png');
    loader.add('bg_attack_click','/assets/char/bg_attack_click.png');
   
    loader.add('menu','/assets/char/myTexture2darrow-resources.assets-279.png');
    loader.add('menu_click','/assets/char/myTexture2darrow2-resources.assets-910.png');
    loader.add('bg_chat','/assets/char/myTexture2dchat-resources.assets-101.png');
    loader.add('bg_chat_click','/assets/char/myTexture2dchat2-resources.assets-430.png');
    loader.add('bg_chat_pc','/assets/char/chat-resources.assets-749.png');
    loader.add('bg_chat_pc_click','/assets/char/chat2-resources.assets-1613.png');
  
    loader.add('bg_info','/assets/char/myTexture2dpanel-resources.assets-1096.png')
    loader.add("head", "/assets/char/9208.png");
    loader.add("body", "/assets/char/581.png");
    loader.add("foot", "/assets/char/596.png");
    loader.add("m2","/assets/map/src/2bgItem0.png")
    loader.add('m3','/assets/map/src/x22$23.png')
   loader.add('m4','/assets/map/src/x2b01.png')
  loader.add('m5','/assets/map/src/x2b112-1.png')
  loader.add('hoa','/assets/map/src/x2blend14layer3.png')
  loader.add('nuida','/assets/map/src/x2b00.png')
  loader.add('click','/assets/map/src/x2blend96layer3.png');
  
  loader.add('giaotiep','/assets/char/giaotiep.png');
  loader.add('zeni','/assets/char/4028.png');
  loader.add('516','/assets/char/516.png');
  
  loader.add('389','/assets/item/389.png');
  
   
  
  loader.add('kimcuong','/assets/char/Small189.png');
  
  base_skill.forEach((item) => {
    let name = item.avatar;
  
    assets.push({name: name, url: '/assets/char/' + name + '.png'});
   
  });
   
   skill.forEach((item) => {
      item.src.forEach((src) => {
          src.start.src.forEach((fam) => {
              let name = item.name + "_" + src.level+ "_" + fam;
  
              assets.push({name: name, url: '/assets/char/' + fam + '.png'});
              
          });
  
          if(src.follow) 
          src.follow.src.forEach((fam) => {
            let name = item.name + "_" + src.level+ "_" + fam;
    ...
    images.forEach((image) => {
      image.farm.forEach((farm) => {
        farm.move.farme.forEach((farme) => {
          // check exits name in VAR cache
  
          assets.push({name: image.name + "_" + farme, url: '/assets/char/' + farme + '.png'});
         
        });
  
       
  
        farm.attack.farme.forEach((farme) => {
        ...
        if(farm.kamejoko)
        farm.kamejoko.farme.forEach((farme) => {
          // check if the resource exists in PIXI Loader
        ...
        if(farm.choang)
        farm.choang.farme.forEach((farme) => {
          // check if the resource exists in PIXI Loader
        ...
        if(farm.gong)
        farm.gong.farme.forEach((farme) => {
  
          assets.push({name: image.name + "_" + farme, url: '/assets/char/' + farme + '.png'});
          
        });
        if(farm.dotay)
        farm.dotay.farme.forEach((farme) => {
          // check if the resource exists in PIXI Loader
  ...
  base_skill.forEach((item) => {
    let name = item.avatar;
    if (!checkCache(name)) {
      loader.add(name, "assets/char/" + name + ".png");
      cache.push(name);
    }
  });
   ...
   skill.forEach((item) => {
      item.src.forEach((src) => {
          ...
          src.start.src.forEach((fam) => {
              let name = item.name + "_" + src.level+ "_" + fam;
              if (!checkCache(name)) {
              loader.add(name, "assets/char/" + fam + ".png");
              cache.push(name);
              }
          });
