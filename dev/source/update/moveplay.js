...
class snowlyvnCreatePlayerOnMap extends snowlyMove {
    constructor() {
        super();
    ...
    snowlyProcessLoadPlayer = () => {
        ...
        for(let i = 0; i < this.Charset.length; i++)
        {
            ...
            if(mmo.type == 'zone')      this.snowlyvnXuLyZone(mmo);
            else if(mmo.type == 'item') this.snowlyvnXuLyItem(mmo);
            else if (mmo.type == 'mob') this.snowlyXuLyMob(mmo);
            else                        this.snowlyvnXuLyPlayer(mmo);
...
export default snowlyvnCreatePlayerOnMap;