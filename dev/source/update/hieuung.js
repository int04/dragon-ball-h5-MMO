import { snowlyvnUpdateSprite } from "./actionplay.js";
class snowlyVNUpdateSkillEFF extends snowlyvnUpdateSprite 
{
    constructor() {
        super();
    ...
    init  = () => {
        ...
        dataSkill.forEach((element) => {
            ...
            if (element.type == "quacauKenhKi") {
              ...
              let targetTo = element.to == this.my.id ? this.NhanVat : this.nguoichoi.getChildByName(element.to);
              if(!targetTo) return element.type = "delete";
        
        
              let wm = Math.abs(mucTieu.width);
              donDanh.visible = false;
              donDanh.docao = mucTieu.y - targetTo.height;
              donDanh.timerungdong = 0.1;
              donDanh.width = 130;
              donDanh.height = 130;
              ...
              donDanh.texture = donDanh.time%this.fps() == 0 ? this.coverImg(startSkill.src[donDanh.start]) : donDanh.texture;
              donDanh.start = donDanh.time%this.fps() == 0 ? donDanh.start+1 : donDanh.start;
              donDanh.start = donDanh.start >= soKhung ? 0 : donDanh.start;
        
              donDanh.timeSet = 50 * this.fps();
              let timeSET = donDanh.timeSet;
              donDanh.x = donDanh.time < timeSET ?  mucTieu.x - donDanh.width / 2 - Math.abs(mucTieu.width) / 2 : donDanh.x;
              donDanh.y = donDanh.time < timeSET ? mucTieu.y - donDanh.height / 2 - mucTieu.height : donDanh.y;
        
              (donDanh.time >= timeSET && element.aim == this.my.id && this.my.info.act == "dotay") && (timeRoiTuDo = 3);
        
              donDanh.ytonao = donDanh.ytonao || this.NhanVat.y - this.NhanVat.height*2;
              let ammin = donDanh.ytonao ;
        
              ((donDanh.time % this.fps() == 0 &&  donDanh.time < timeSET && element.aim == this.my.id && this.my.info.act != "dotay" && this.NhanVat.y > ammin)  && (keysPressed[38] = true)  && (since04.to({
                move: {
                  update: {
                    x: this.NhanVat.x,
                    y: this.NhanVat.y,
                    move: this.my.info.move ? this.my.info.move : "right",
                  },
                },
              })));
        
              if(element.aim == this.my.id && this.my.info.act != "dotay" && this.NhanVat.y <= ammin) {
                keysPressed[38] = false;
                this.my.info.act = "dotay";
                this.addAction({id : this.my.id, action : 'dotay'});
                since04.to({
                  move: {
                    update: {
                      x: this.NhanVat.x,
              ...
              let xTO = targetTo.x - donDanh.width;
              let speedt = 1;
              if (donDanh.time >= timeSET) {
                if (donDanh.x !== xTO) {
                donDanh.x += donDanh.x < xTO ? Math.min(speedt, xTO - donDanh.x) : -Math.min(speedt, donDanh.x - xTO);
                }
                
                if (donDanh.y !== targetTo.y) {
                donDanh.y += donDanh.y < targetTo.y ? Math.min(speedt, targetTo.y - donDanh.y) : -Math.min(speedt, donDanh.y - targetTo.y);
                }
                
                if (donDanh.x === xTO && donDanh.y === targetTo.y) {
                donDanh.visible = false;
                element.type = "delete";
                ...
                if (element.aim === this.my.id && element.keyid || (element.detu == true && this.my.detu && this.my.detu.id && this.my.detu.id == element.aim )) {
            ...
            if (element.type == "eff_fly_now") {
              ...
              if (this.kiNang.getChildByName(element.id) == undefined) {
                ...
                let bg_imgLoad = new PIXI.Sprite(this.coverImg(startSkill.src[0]));
        
                let arrayFrame = [];
                let yyget = 0;
                let yOne = startSkill.height / startSkill.khung;
                for (let i = 0; i < startSkill.khung; i++) {
                  arrayFrame.push(
                    new PIXI.Texture(
                      bg_imgLoad.texture,
                      new PIXI.Rectangle(0, yyget, startSkill.width, yOne)
                    )
                  );
                  yyget += yOne;
                }
                let donDanh = new PIXI.AnimatedSprite(arrayFrame);
                donDanh.animationSpeed = 0.3;
        
                donDanh.play();
        
                donDanh.x = element.startX;
                donDanh.y = element.startY;
                donDanh.start = 0;
                donDanh.time = 0;
                ...
                if (element.aim == this.my.id) {
                  ...
                  phanDau = mucTieu.getChildByName("dau");
                  phanChan = mucTieu.getChildByName("quan");
                } // ! mục tiêu là người chơi, npc khác
                else {
                  mucTieu = this.nguoichoi.getChildByName(element.aim);
                  phanDau = mucTieu.getChildByName("playerDau");
                  phanChan = mucTieu.getChildByName("playerQuan");
                }
        
                donDanh.width = phanDau.width * 3;
        
                if (mucTieu != undefined) {
                  if (mucTieu.move == "right") {
                    donDanh.scale.x = -1;
                    donDanh.pivot.x = donDanh.width;
                  } else {
                    donDanh.scale.x = 1;
                    donDanh.pivot.x = -donDanh.width / 2;
                  }
        
                  if (mucTieu.act == "move") element.type = "delete";
        
                  if (donDanh.time % 5 == 0) {
                    donDanh.x = mucTieu.x - phanDau.width - donDanh.width / 2;
                    donDanh.y = phanChan.y + mucTieu.y - phanDau.height / 2;
                    // quay ảnh xuống dưới
                    // set width, height
                    //  donDanh.scale.set(startSkill.height, startSkill.width);
                    // donDanh.rotation = Math.PI / 2;
                  }
                } else {
                  addnewskill.removeChild(donDanh);
                  this.kiNang.removeChild(addnewskill);
                  dataSkill = dataSkill.filter(function (obj) {
                    return obj.id !== element.id;
                  });
                }
              }
            ...
            if (element.type == "eff_fly") {
              ...
              donDanh.texture = donDanh.time%this.fps() == 0 ? this.coverImg(startSkill.src[donDanh.start]) : donDanh.texture;
              donDanh.start = donDanh.time%this.fps() == 0 ? donDanh.start+1 : donDanh.start;
              (donDanh.start >= soKhung) && (donDanh.start = 0);
            ...
            if (element.type == "skill_one") {
              ...
              if(donDanh.x == mucTieu.x && donDanh.y == mucTieu.y) 
              {
                ...
                if (
                  element.by &&
                  element.by == this.my.id &&
                  this.my.info.act == "kamejoko"
                ) {
                  this.resetAction(this.my.id)
                }
              }
            ...
            if (element.type == "truhp") {
              ...
              donDanh.time = donDanh.time+1 || 0;
              donDanh.visible = true;
              let ChoPlayer = this.my == element.to ? this.my : this.Charset.find(item => item.id == element.to);
              if(donDanh.time <=3) 
              {
                if (ChoPlayer) {
              ...
              (donDanh.y < mucTieu.y - 80) && (element.type = 'delete');
            ...
            if (element.type == "truki") {
              ...
              (donDanh.y < mucTieu.y - 130) && (element.type = 'delete');
...
export default snowlyVNUpdateSkillEFF;