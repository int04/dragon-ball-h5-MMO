import snowlyvnTalkBox from './giaotiep.js';

/**
 * @class snowlyvnVutItemClass
 * @extends {snowlyvnTalkBox}
 * @desc : Xử lý vứt đồ
 * !!! Các hàm chính
...
class snowlyvnVutItemClass extends snowlyvnTalkBox 
{
    constructor() {
        super();
        this.item = this.item || {};
    ...
    ItemNhat = (data) => {
        (this.TimeNhatDo < Date.now()) && ( this.IoNhatItem(data) , this.TimeNhatDo = Date.now() + 1000);
    };

    comfirmVutItem = (data) => {
        if(this.my.id <=0) return;
        let inBag = this.findBag(data);
        if(!inBag) this.notice(this._('Có lỗi xẩy ra.'));

        if(inBag.active == "trangbi") this.notice(this._('Vui lòng tháo món đồ ra trước khi vứt.'));

        let infoItem = this.findItem(inBag.item);
        if(!infoItem) return this.notice(this._('Có lỗi xẩy ra.'));

        if(infoItem && infoItem.camvut && infoItem.camvut == true) return this.notice(this._('Không thể vứt vật phẩm này.'));
        if(inBag && inBag.khoa && inBag.khoa == 1) return this.notice(this._('Không thể vứt vật phẩm này.'));

        if(infoItem.type2 == 'caitrang' ) 
        {
            this.menuComfirmVut(data);
        }
        else 
        {
            this.submitVutItem(data);
        }
    }
    ...
    menuComfirmVut = (data) => {
        ...
        let text = new PIXI.Text(this._('Bạn có muốn vứt vật phẩm này không ? (sẽ bị mất luôn.)'), {
            fontSize: 16,
            fill: 0x532905,
            fontFamily: 'chelthm',
            fontWeight: 'bold',
...
export default snowlyvnVutItemClass;