import SheetImges from './sheet.js';
export default class source extends SheetImges {
    constructor() {
        super();
        this.images = [];
        this.assets = [];
    ...
    load (link) {
        return PIXI.Assets.load(link);
    }

    getUrlImg(name, folder = 'char') {
        // check Sheet 
        let sheet = this.checkSheet(name);
        if(sheet) return './assets/char/' + sheet.sheet + '.png';
