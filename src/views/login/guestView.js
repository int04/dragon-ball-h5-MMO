import loadingView from './loadView.js';
class guestView extends loadingView {
    constructor() {
        super();
        this.playSound("music2",true,null,"musicwelcome")
    }
    ...
    createBackgroundMap = () => {
        ...
        for(let i = 0; i < Math.round(this.gameWidth / 258) * 2; i++) {
            let base = this.createMapNui(i * 258, 0, 'x2b03', 258, this.gameHeight * 0.15);
        ...
        for(let i = 0; i < Math.round(this.gameWidth / 510) * 2 + 6; i++) {
            let base = this.createMapNui(i * 300, 0, 'x2b02', 300, this.gameHeight * 0.30);
        ...
        for(let i = 0; i < Math.round(this.gameWidth / 336) * 2 + 3; i++) {
            let base = this.createMapNui(i * 336, 0, 'x2b01', 336, this.gameHeight * 0.30);
            nuidaxa.addChild(base);
        }
        nuidaxa.y = nuidaxanua.y + nuidaxanua.height/2 + 30;


        let nuida = new PIXI.Container();
        nuida.name = "load1";
        let soluong = Math.round(this.gameWidth / 336) * 2 + 6;
        let ynui = this.gameHeight - this.gameHeight * 0.35;
        let heightnui = this.gameHeight * 0.3;
        for(let i = 0; i < soluong; i++) {
            let nui = this.createMapNui(i * 336, 0, 'x2b00',336, heightnui);
            nuida.addChild(nui);
        }
    ...
    CreateMainGuestGame = () => {
        ...
        let desc = new PIXI.Text('*:Trò chơi là đồ án của sinh viên Công nghệ thông tin. Trò chơi là dạng mã nguồn mở (được chia sẻ cho mọi người cùng học, tham gia đóng góp). Chức năng nạp tiền sử dụng để duy trì cho dự án, không mục đích kinh doanh.', {
            fontFamily: 'Arial',
            fontSize: 15,
            fill: 0xFFFFFF,
            fontWeight: 'bold',
        ...
        // Client with no official copyright - developed by open source communities
        let copyright = new PIXI.Text(
            '© "Client" - không thuộc bản quyền của bất kì tổ chức hay cá nhân nào ! - "Trò chơi" được viết trên mã nguồn do cộng đồng đóng góp, sử dụng hình ảnh từ nhiều nguồn trên Internet.', {
                fontFamily: 'Arial',
                fontSize: 16,
                fill: 0x000000,
                fontWeight: 'bold',
        ...
        copyright.style.strokeThickness = 2;
        copyright.y = 0;
        copyright.x = 0;
        ...
        setTimeout(() => {
            if(this.username != '' && this.gameInfo.login ) this.logInGame();
            //this.pageLogin();
            setTimeout(() => {
            }, 200);
        }, 1);
