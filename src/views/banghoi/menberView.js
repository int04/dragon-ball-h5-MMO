import previewMenberBT_ from './previewMenber.js';

export default class menberPTView extends previewMenberBT_ {
    constructor() {
        super();
    }
    ...
    boxPTMenber = () => {
        ...
        let menuBox = [
            {name : "Chat\nBang", function : 'boxNewBangHoi'}, 
            {name : 'Thành\nviên', function : 'boxPTMenber'}, 
            {name : 'Xin vào'+(this.banghoi.xinvao.length >=1 ? '('+this.banghoi.xinvao.length+')' : ''), function : 'boxPTMenberXin', right : '1', 'type' : '>='}, 
            {name : "Rời", function : 'outBangHoi', right : 1, 'type' : '<'}, 
            {name : "Biểu tượng", function : 'boxPTShopBieuTuong', right : 2, 'type' : '>='},
        ];

        let meRight = this.banghoi.menber.find(e => e.id == my.id).right;
        let box1 = new PIXI.Container();
        hienthinoidung.addChild(box1);
        let count = 0;
        menuBox.forEach((element,i) => {
            if(element.type == '>=' && meRight < element.right) {
                return;
            }
            if(element.type == '<' && meRight > element.right) {
                return;
            ...
            background.x = width * count + space * count;
        ...
        let txt = this.html(`<font color="532905">`+'Danh sách thành viên'+`</font>`);
        ...
        // sort DESC time
        this.banghoi.menber.sort((a,b) => {
            return b.sucmanh - a.sucmanh;
        });

        bang.menber.forEach((element,i) => {
            ...
            let name = this.html(`<font color="`+(element.right == 2 ? 'red' : element.right == 1 ? 'blue' : 'black')+`">`+element.name+`</font>`);
            ...
            background.on('pointerup', () => {
                let time2 =  Date.now();
                if(time2 - time < 200) return this.previewMenberOffcial(background,element);  
            });
