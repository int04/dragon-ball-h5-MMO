...
class profileView extends shopView {
    ...
    boxNewNhiemVu = () => {
        ...
        if(!dataNhiemVu) 
        {
            noidung += "<center><b>Trái đất đã hoà bình</b></center> <br>";
            noidung += "<b>- Sau những ngày tháng gian nan vất vả, cuối cùng trái đất của chúng ta cũng được hoà bình.</b>";
            noidung +="<br><b>- Hãy tận hưởng cuộc sống hoà bình của thế giới Ngọc Rồng, tham gia sự kiện, hoạt động cùng bạn bè.</b>";
            noidung +="<br><b>- Không nên chủ quan khi đang hoà bình, hãy tích cực luyện tập để tương lai có thể đương đầu với những kẻ thù mạnh hơn.</b>";
            noidung+="<br> <b> - Cám ơn bạn đã tham gia cuộc hành trình bảo vệ trái đất.</b>";
        }
        else 
        {
            noidung +=`<center><font color="black"><b><big style="font-size:23px">`+dataNhiemVu.name+`</big></b></font></center><br><br><br>`;
            noidung += `<b style="font-size:18px;">`;
            dataNhiemVu.list.forEach((element,i) => {
                let font = '';
                if(this.my.nhiemvu.data.have >= this.my.nhiemvu.data.need && this.my.nhiemvu.data.type != 'talk') 
                {
                    this.my.nhiemvu.now += 1;
                } 
                if(i < this.my.nhiemvu.now) font = '#ffff';
                if(i == this.my.nhiemvu.now) font = 'blue';
                if(i > this.my.nhiemvu.now) noidung+= '<br><b>- ...</b>';
                else noidung+=`<br><b><font color="`+font+`">- `+element.name+``+(element.type != 'talk' ? `(`+this.my.nhiemvu.data.have+`/`+this.my.nhiemvu.data.need+`)` : '')+`</font></b>`;
            });
            noidung +=`</b>`;
            noidung+=`<br><br><br> <b style="font-size:17px">`+dataNhiemVu.desc+`</b> <br> `;
        }

        let hienthinoidung2 = new PIXI.HTMLText(noidung, {
            fontSize: 16,
            fontWeight: '900',
            align: 'left',
    ...
    boxNewChucNang = () => {
        ...
        {
            name : 'Chức năng',
            action : 'runBoxSetting',
        },


            {
                name: 'Thoát',
                action: 'outGame,closeBox',
        }, 

        {
            name : 'Liên kết tài khoản',
            action : 'pageActiveAccount',
        },
        ...
        for(let i = 0; i < arrayMenu.length; i++) {
            ...
            if(arrayMenu[i].action == 'pageActiveAccount' && this.my.veri == 1 ) { 
                slot.visible = false;
            }
            ...
            slot.on("pointerup", function(event) {
                ...
                if(pointerEndTime - pointerStartTime < 200) {
                    ...
                    list.forEach(element => {
                        if(arrayMenu[i].action == 'pageActiveAccount' && self.my.veri == 1 ) {
    ...
    boxNewKyNang = () => {
        ...
        listSkill.forEach(element => {
            ...
            if(!f) my.skill.push({ id: element.id, level: 0, xep : element.xep }); 
        });
        let myskill = my.skill;
        // sort ASC ID 
        myskill.sort(function(a, b) { return a.xep - b.xep });
        let iShow = 0;
    ...
    boxNewBag = () => {
        ...
        for(let i = 0; i < oTrangBi; i++) {
            ...
            BgBolderAvatar.drawRoundedRect(0, 0, listItem.maxWight * 0.15, listItem.maxHeight, 0);
            ...
            BgBolderInfo.drawRoundedRect(slotwidth*0.16, 0, listItem.maxWight * 0.84, listItem.maxHeight, 0);
            ...
            if(my.trangbi[fewUse[i]] && my.trangbi[fewUse[i]] != 0) {
                ...
                if(inBag) {
                    ...
                    if(inItem) {
                        ...
                        showImgItem.width = slotheight*0.6;
                        showImgItem.height = slotheight*0.6;
