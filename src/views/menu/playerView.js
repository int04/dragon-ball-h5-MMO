import previewShopView from '../preview/shopView.js';

export default class playerView extends previewShopView {
    constructor() {
        super();
    }
    /**
    ...
    viewInfoPlayer = (id, chon = 0) => {
        ...
        for(let i = 0; i < arrayMenu.length; i++) {
            ...
            if(arrayMenu[i].action == 'moipt') {
                let me = this.my;
                if(me.skin.bangID <=0) continue;
                if(my.skin.bangID >=1) continue;
                let checkRIGHT = this.banghoi.menber.find(e => e.id == me.id).right;
                if(checkRIGHT < 1) continue;
            }
            ...
            slot.on("pointerup", function(event) {
                ...
                if(pointerEndTime - pointerStartTime < 200) {
                    ...
                    (arrayMenu[i].action == 'moipt') && self.PT_Moi_Vao_Bang(id);
                    (arrayMenu[i].action == 'thachdau') && self.formInputPkView(id);
                } else {}
            });
