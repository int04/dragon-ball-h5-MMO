...
class player extends msgBox {
    constructor() {
        super();
        this.checkSpriteCreate = 0;

    }

    TaoSprite2 = () => {
        if(this.checkSpriteCreate ==0 ) {
            this.TaoSprite();
            this.createDisplayOnPlayerInfo();

            this.checkSpriteCreate = 1;
        ...
        for(let i = 0; i < 1; i++) {
            ...
            if(objectTest.skin.theobo && objectTest.skin.theobo.length >=1) 
            {
                ...
                if(findnameAo) objectTest.skin.ao = findnameAo.name;
                let findnameQuan = this.images.find(e => e.id == objectTest.skin.theobo && e.type == 'quan');
                if(findnameQuan) objectTest.skin.quan = findnameQuan.name;
                let findnameDau = this.images.find(e => e.id == objectTest.skin.theobo && e.type == 'dau');
                if(findnameDau) objectTest.skin.dau = findnameDau.name;
            }
