import attackDetuView from './detu.js';
class attackView extends attackDetuView {
    constructor() {
        super();
    }
    ...
    reposiveAttack(data) {
        ...
        if(data.id == 1 || data.id == 15 || data.id == 16) {
            // đám dragon
            ...
            let my2 = this.getMy(from);
            if(my2.eff && my2.eff.hoakhi && my2.eff.hoakhi.active == true) 
            {
                this.addEff({
                    type: 'hoakhi_dondam',
                    keyid: data.keyid,
    ...
    interAttack() {
        ...
        if(this.my.info.chiso.hp <= 0) return this.boxMenuHoiSinh();
        let my = this.my;
        ...
        if(infoSkill) {
            ...
            // kiểm tra khoảng cách
                if(infoSkill.dx && infoSkill.dx > 0 && database) {
                    let nhanVat = this.NhanVat;
                    let NhanVat2 = this.getNhanVat(database.id);
                    let distance = this.calculateDistanceXandWidth(nhanVat, NhanVat2);

                    if(distance > infoSkill.dx*2) {
                        console.log('Quá xa',distance)
                        this.goto = true;
                        this.gotoDX = infoSkill.dx;
            ...
            if(infoSkill.type == 'attack' && database && database.info && database.info.chiso && database.info.chiso.hp <=0) return this.bug('mục tiêu đã chết'); // kiểm tra mục tiêu đánh là tấn công, đồng thời chết
           ...
           // findskill.time = Date.now() + (infoSkill.time[lvKilluse] * 1000);
            ...
            serverCode = database && database.type == 'boss' ? 3 : serverCode;
