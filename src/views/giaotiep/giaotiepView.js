import giaotiepAction from './giaotiepAction.js';
class giaotiepView extends giaotiepAction {
    ...
    talkWith(id) {
        ...
        if(npc.id == this.my.nhiemvu.data.id)
        {
            return this.nhiemvuView(0);
        }

        if(npc.pos.x < this.NhanVat.x) {
            npc.info.move = 'right';
        }
        if(npc.pos.x > this.NhanVat.x) {
            npc.info.move = 'left';
        ...
        return this.talkWithButton(source.giaotiep, source.action, source.script.avatar,id)
    }

    talkWithButton(msg, buttonListIN = [], avatar = '',id = null) {
        this.closeBox();
            ...
            button.on("pointerup", function(event) {
                ...
                if(pointerEndTime - pointerStartTime < 200) {
                    if(buttonlist[i].a == 'close') {
                        ...
                        self.talkAction(buttonlist[i].a,buttonlist[i]);
                        self.khungGiaoTiep.visible = false;
                        self.khungGiaoTiep.removeChildren();
                    }
                } else {}
