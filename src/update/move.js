import keypressMove from '../views/keybord/keypress.js';
class moveSprite extends keypressMove {
    constructor() {
        super();
        this.updateUserTimeRender = 0;
        this.updateCreatePlayerOnMap();
        this.debut = ["thaiduonghasan","rungu"];

    }

    updateCreatePlayerOnMap = () => {
        clearTimeout(this.updateUserTimeRender);
        requestAnimationFrame(this.ProcessLoadPlayer);
    };
        ...
        if(self.nguoichoi.getChildByName(mmo.id) == undefined) {
            ...
            if(ItemBG == undefined) {
                ...
                */
                ...
                ItemBG.on('pointerdown', (infoclick) => {
                    self.clickPlayer({
                        id: vatpham.id
                    });
                })
    ...
    XuLyMob = (mmo) => {
        ...
        if(self.nguoichoi.getChildByName(mmo.id) == undefined) {
            ...
            if(load_playerAo.farm[0].move.width <=10 || load_playerAo.farm[0].move.height <=10) {
                playerAo.scale.set(load_playerAo.farm[0].move.width, load_playerAo.farm[0].move.height);
            }
            else {
                playerAo.width = load_playerAo.farm[0].move.width;
                playerAo.height = load_playerAo.farm[0].move.height;
            }
            ...
            if(Player.checkSieuQuai == 0) 
            {
                if(Player.getBounds().width >= 12) 
                {

                    if(mmo.info.coban && mmo.info.coban.sieuquai && mmo.info.coban.sieuquai != -1 && mmo.info.chiso.hp > 0) 
                    {
                        let nameimg = {
                            saiyan : [964,965,966,967,982,983,984],
                            traidat : [971,972,973,974],
                            namek : [977,978,986]
                        };
                        this.addEff({
                            type: 'loop',
                            aim: mmo.id,
                            id : 'sieuquai'+mmo.id,
                            src: nameimg[mmo.info.coban.super],
                            width: 90,
                            height: 90,
                        });
                    }
                    Player.checkSieuQuai = 1;
                }
            }
            ...
            if(Player.visible == false && mmo.info.chiso.hp >0)
            {
                Player.visible = true;
            }
            if(player_phukien) {
    ...
    XuLyPlayer = (mmo) => {
        ...
        if(self.nguoichoi.getChildByName(mmo.id) == undefined) {
            ...
            if(Player.type == 'boss') 
            {
                ...
                if(mmo.info.namdat == 1 && mmo.info.act == 'baylen') 
                {
                    if(this.checkOnDat22(mmo.id,'up')) {
                        this.action(mmo.id,'dungyen');
                    }
                    else 
                    {
                ...
                if(mmo.info.namdat == 1 && (mmo.info.act == 'dungyen' || mmo.info.act == 'dotay')) 
                {
                    mmo.info.act = 'dungyen';
                    this.xulyBOSS(Player, mmo);

                }

                let doxa = Math.abs(yNew - yOld);
                if(doxa <= 50) 
                {
                    yNew = yOld;
                }
    ...
    XuLyZone = (mmo) => {
        ...
        if(objectZone == undefined) {
            ...
            let dx = this.calculateDistanceXandWidth(this.NhanVat, objectZone);
            if(dx < 100) objectZone.visible = true;
            else objectZone.visible = false;

            if(objectZone.visible == true) {
                this.setting.mouse = objectZone.id;
            }
            let can = 0;
            ...
            if(mmo.auto) {
                if(this.nguoichoi.getChildByName(mmo.id + '_auto') == undefined) {
                    ...
                    let height = 200;
                    if(mmo.all == true) height = this.gameMap.setting.maxX;
                    if(height != 200) 
                    {
                        mmo.pos.x = this.gameMap.setting.minX;
                    }
                    Graphics2.name = mmo.id + '_auto';
                    Graphics2.id = Graphics2.name;
                    Graphics2.type = mmo.type;
                    Graphics2.beginFill(0x000000, 0.5);
                    Graphics2.drawRect(0, 0, height, 50);
                    Graphics2.endFill();
    ...
    ProcessLoadPlayer = () => {
        ...
        if(this.updateUserTimeRender % this.fps()) return;
        let self = this;
        ...
        for(let i = 0; i < this.Charset.length; i++) {
            ...
            if(mmo.type == 'zone') this.XuLyZone(mmo);
            else if(mmo.type == 'item') this.XuLyItem(mmo);
            else if(mmo.type == 'mob') this.XuLyMob(mmo);
            else this.XuLyPlayer(mmo);
    ...
    xulyNPCOnMap(Player, mmo) {
        ...
        if(this.my.id >=1 &&  mmo.info.right ) {
            if(this.NhanVat.x > mmo.pos.x) mmo.info.move = 'right';
            else mmo.info.move = 'left';
        }
        
        if(Player.npcChat < Date.now() && this.my.nhiemvu.data.id != mmo.id && this.Chat.children.findIndex(e => e.uid == Player.id) == -1 ) {
        ...
        if(Player.getBounds().height <=0 || Player.getBounds().width <=0) return console.log('Chưa định hình cấu');
        ...
        Promise.all(
            ...
            datd.map((dat, index) => {
                ...
                return new Promise((resolve, reject) => {
                    if(player.getBounds().height <=0 || player.getBounds().width <=0 ) resolve(false);
                    if(dat.getBounds().height <= 0 || dat.getBounds().width <= 0) resolve(false);
                    if(dat.renderable == false) resolve(false);
            ...
            if(k != null) {
                mmo.pos.y = Math.round(datd[k].y - player.getBounds().height + 10);
                mmo.pos.y = Math.round(mmo.pos.y);
                Player.y = mmo.pos.y;
                Player.checked += 1;

               // console.log(datd[k].y , player.height, datd[k].getBounds())
    ...
    xulyMobOnMap(Player, mmo) {
        ...
        if(Player.checked >= 1) return false;
        if(Player.getBounds().height <=10 || Player.getBounds().width <=10) return console.log('Chưa định hình cấu');
        ...
        Promise.all(
            ...
            datd.map((dat, index) => {
                ...
                return new Promise((resolve, reject) => {
                    if(player.getBounds().height <=10 || player.getBounds().width <=10 ) resolve(false);
                    if(dat.getBounds().height <= 10 || dat.getBounds().width <= 10) resolve(false);
                    if(dat.renderable == false) resolve(false);
                    let dx = this.calculateDistance(player, dat);
                    if(gannhat == null || gannhat > dx) {
                        gannhat = dx;
            ...
            if(k != null) {
                let old = mmo.pos.y;
                mmo.pos.y = Math.round(datd[k].y*1 - Math.abs(player.height) + 24);
                mmo.pos.y = Math.round(mmo.pos.y);
                Player.y = mmo.pos.y;
                Player.checked += 1;
                //console.log('dat', datd[k].y ,'old', old, 'new', mmo.pos.y, 'height',player.height)
    ...
    xulyBOSS(Player, mmo) {
        ...
        let datd = this.bando.children.filter(item => item.type === "dat");
        if(datd.length <= 0) return console.log('Lỗi dất');

        let data = mmo;
        let player = Player;
        if(!player) return console.log('Lỗ iowr đây');
