import { iohandle } from "./handle.js";

class ioMethod extends iohandle {
    constructor() {
        super();
    }
    ...
    IoEvent = (args) => {
        ...
        if(!!data.chipi) this.chipi(data.chipi);
        ...
        if(!!data.login) {
            if(data.login.status) {
                ...
                let objectTest = this.my;
                console.log(objectTest.skin)

                if(objectTest.skin.theobo && objectTest.skin.theobo.length >=1) 
                {
                    console.log(objectTest.skin.theobo)
                    let findnameAo = this.images.find(e => e.id == objectTest.skin.theobo && e.type == 'ao');
                    if(findnameAo) this.my.skin.ao = findnameAo.name;
                    let findnameQuan = this.images.find(e => e.id == objectTest.skin.theobo && e.type == 'quan');
                    if(findnameQuan) this.my.skin.quan = findnameQuan.name;
                    let findnameDau = this.images.find(e => e.id == objectTest.skin.theobo && e.type == 'dau');
                    if(findnameDau) this.my.skin.dau = findnameDau.name;

                }

                this.LoadAssetPlayer();
                this.guestContainer.visible = false;
                this.loadGame.visible = true; // màn hình load game// chờ loading map
